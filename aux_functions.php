<?php
  ...
  function get_filtered_actors($d, $actors, $pfa) {
    $fa = array_map('strtolower', $actors);
    $go_back = isset($d['going-back']) ? intval($d['going-back']) : 0;
    if (isset($d['fa']) and (!empty($d['fa'])) and (!$go_back)) {
      $fa = array_map('strtolower', $d['fa']);
    }
    if ( ! empty($pfa) and $go_back) {
      $fa = array_map('strtolower', $pfa);
    }
    return $fa;
  }
  
  function get_filtered_topics($d, $topics, $pft) {
    $ft = array_map('strtolower', $topics);
    $go_back = isset($d['going-back']) ? intval($d['going-back']) : 0;
    if (isset($d['ft']) and !empty($d['ft']) and (!$go_back)) {
      $ft = array_map('strtolower', $d['ft']);
    }
    if ( ! empty($pft) and $go_back){
      $ft = array_map('strtolower', $pft);
    }
    return $ft;
  }
  ...
  function get_past_filters($d, $key, $default) {
    ...
    if (isset($d[$key]) and !empty($d[$key])) {
      return explode($sep, $d[$key]);
    } else {
      return array_map('strtolower', $default);
    }
  }
